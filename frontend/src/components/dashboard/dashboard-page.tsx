...
// Cleared only on logout (authService) or hard refresh.
let _cachedPortfolio: PortfolioResponse | null = null;
      ...
      <div className="relative bg-card border border-border rounded-xl overflow-hidden
        ...
        <div className="flex items-center justify-between px-5 py-3
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-semibold text-foreground">Portfolio Pulse</span>
            {city && (
          ...
          <Link href="/profile" className="text-xs text-primary hover:underline font-medium">
            Edit profile
          </Link>
        </div>
        <div className="grid grid-cols-3 divide-x divide-border-subtle">
          {stats.map(({ icon: Icon, label, value, sub, valueColor, iconColor, iconBg }) => (
            <div
              key={label}
  ...
  return (
    ...
    >
      ...
      <div className="relative flex items-start justify-between">
        <div className={cn(
          "h-11 w-11 rounded-xl flex items-center justify-center shrink-0 shadow-(--shadow-xs)",
          iconBg, iconColor,
        )}>
...
export function DashboardPage() {
  // If we've already loaded once this session, skip the loader entirely.
  const [isLoading, setIsLoading] = useState(!_alreadyLoaded);
  const [portfolio, setPortfolio] = useState<PortfolioResponse | null>(_cachedPortfolio);

  const user = authService.getStoredUser();
  const firstName = user?.full_name?.split(" ")[0];

  useEffect(() => {
    // Already have data from a previous mount — nothing to do.
