...
def make_couple(**ov) -> CoupleProfile:
    return CoupleProfile(
        ...
        is_married=ov.get("is_married", True),
        joint_goals=ov.get("joint_goals", []),
    )
...
class TestNetWorth:
    def test_net_worth_assets_minus_liabilities(self):
        p = make_profile(
            ...
            debts=[DebtItem(name="L", outstanding=200_000, emi=5_000, interest_rate=10)],
        )
...
class TestHRABenefit:
    def test_metro_hra_gte_non_metro(self):
        assert _hra_benefit(make_profile(city="Mumbai")) >= _hra_benefit(
            make_profile(city="Jaipur")
        )
...
class TestCoupleOptimisation:
    def test_combined_net_worth_sum_of_both(self):
        ...
        r = optimise_couple_finances(
            CoupleProfile(partner_a=a, partner_b=b, is_married=True, joint_goals=[])
        )
